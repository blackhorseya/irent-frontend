          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              ...
              {booking.loading === false && (
                  booking.error && (
                      <Typography>{booking.error}</Typography>
                  )
              )}
              {booking.loading === false && booking.data && (
                  <List>
                    {booking.data.map((item, _) =>
                        <ListItem key={item.no}>
                          <ListItemText id={item.no} primary={item.car_id}/>
                          <ListItemText id={item.no}
                                        primary={formatTime(
                                            item.stop_pick_at)}/>
                          <ListItemIcon>
                            <IconButton onClick={() => this.handleOpenMaps(
                                item.car_latitude, item.car_longitude)}>
                              <Room/>
                            </IconButton>
                          </ListItemIcon>
                          <ListItemIcon>
                            <IconButton onClick={() => this.handleCancelBooking(
                                item.no)}>
                              <Cancel/>
                            </IconButton>
                          </ListItemIcon>
...
const actionCreators = {
  list: bookingActions.list,
  cancel: bookingActions.cancel,
};

const connectedBookingList = connect(mapStateToProps, actionCreators)(
    BookingList);
